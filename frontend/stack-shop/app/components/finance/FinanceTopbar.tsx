...
export function FinanceTopbar({ title, nodeID, financeName, financeInitial }: { title: string; nodeID: string; financeName: string; financeInitial: string }) {
  return (
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end mr-2">
            <span className="text-xs font-bold text-gray-500">{financeName}</span>
          ...
          <div className="w-10 h-10 rounded-xl bg-gray-200 dark:bg-gray-800 overflow-hidden border-2 border-white dark:border-gray-700 shadow-sm">
            <div className="w-full h-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-black text-xs">{financeInitial}</div>
          </div>
        </div>
      </div>
    </header>
